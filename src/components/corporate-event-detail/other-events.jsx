...
import { corporateEventDetails } from '../../data/corporate-event-details';
import Container from '../ui/container';
import { fadeUp, stagger } from '../ui/animations';

export default function OtherEventsSection({ currentSlug }) {
  const others = corporateEventDetails.filter((e) => e.slug !== currentSlug);
          ...
          {others.slice(0, 4).map((other) => {
            ...
            return (
              <motion.div key={other.slug} variants={fadeUp}>
                <Link
                  to={`/corporate-events/${other.slug}`}
                  className="group block rounded-2xl bg-bg-main border border-border/50 overflow-hidden hover:border-primary/30 transition-all duration-300"
                >
